...
import Editinguser from '../layouts/editinguser';
...
function UpdateForm({ id1,name1,email1,pass1,ph1,eno1,doa1})
  {
    //const navi =useNavigate();
    if(id1!=null){
  ...
  <Editinguser idd={id1} namee={name1} eemail={email1} ppass={pass1} phh={ph1}  eeno={eno1} ddoa={doa1}/>}}
    // setid(user_Id=>idd);
    // setname(user_Name=>namee);
    // setmail(e_mail=>eemail);
    // setph(ph_no=>phh);
    // setpass(pass_word=>ppass);
    // seteno(e_no=>eeno);
    // setdoa(d_oa=>ddoa);
    ...
    // React.useEffect(() => {
    //   axios.get('user/update/'+user_Id).then((response) => {
